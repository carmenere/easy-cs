...
- [Связи с другими темами](#связи-с-другими-темами)
  - [Связь перестановок с вращательной симметрией](#связь-перестановок-с-вращательной-симметрией)
...
$`O(a, \phi) = \{a_{1}, \space a_{2}, \space a_{3}, \dots, a_{k-1}, \space a_{k}\}`$, где $`a_{i}∈M \space \forall i \in [1,k]`$. **Мощность** *орибты* равна $`k`$: $`|O(a, \phi)| = k`$<br>
...
Пусть $`\phi`$ произвольная перестановка на множестве $`M = \{1, 2, 3, ..., n\}`$, которая разбивает его на $`s`$ (при этом $`s \le n`$) **орбит**: $`O_{1}, \space O_{2}, \space \dots, \space O_{s}`$. Тогда каждой **орбите** $`O_{s}`$ соответствует **независимый цикл** (**disjoint cycle**), причем **длина** этого **цикла** равна **мощности** орибты $`|O_{s}| = k_{s}`$.<br>
...
## Связь перестановок с вращательной симметрией
Перестановка:<br>
...
описывает **поворот** любой фигуры обладающией *вращательной симметрией* (**n**-угольника, ожерелья из **n** бусинок, шестерёнки с **n** зубцами) на угол $`{\displaystyle {\tfrac {360^{\circ }}{n}}}`$ вокруг **своего центра** *вращательной симметрии*.<br>
