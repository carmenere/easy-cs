...
Каждому элементу группы $`SO(2)`$ соответсвет определнный угол $`\phi`$ (в радианах), но т.к. все углы, отличающиеся на $`2\pi`$, определяют один и тот поворот окружности, то каждому элементу группы $`SO(2)`$ соответствует не только число $`\phi`$, но и все числа вида $`\phi + k \cdot 2\pi`$, где **k** любое целое число.<br>
